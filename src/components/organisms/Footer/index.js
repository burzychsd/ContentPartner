        ...
        <Flex as='footer' ref={innerRef ? innerRef : null} reset css={tw`bg-dark_puce w-full`} style={{ ...props.style }}>
            <Flex className='footer_container' reset css={tw`w-full h-full justify-between items-center px-8 mx-auto`} style={{ maxWidth: 1280 }}>
                <Flex reset className='footer_links'>
                    <Text reset css={footerTextClasses} style={{ cursor: 'pointer' }} onClick={() => handleClick('FAQ')}>FAQ</Text>
                    <Text reset css={footerTextClasses} style={{ cursor: 'pointer' }} onClick={() => handleClick('Cookies')}>Prywatność</Text>
                    <FooterLink to='/kontakt/'>Kontakt</FooterLink>
                </Flex>
                <Text reset className='footer_copyright' css={footerTextClasses}>&copy;{` ${new Date().getFullYear()} Content Partner`}</Text>
            </Flex>
        </Flex>
    )
})
